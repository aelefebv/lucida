...
- **2026-05-15 → 2026-05-16** — `principles/cpu-cache.md` did not exist. Originally queued to wait for the fetch/decode refactor (PRD #592) to settle before authoring, since principles emerge from the patterns visible in the cleaner code. Resolved at the user's request directly after the refactor PR opened: six principles authored via INTERVIEW (decoded bytes survive GPU eviction; tier-aware eviction protects bytes by replacement cost; movement across tiers cheap, refetch expensive; active-detail evicts least-recently-wanted lowest-importance first; failure is windowed not permanent; one fetch path, one budget, one failure regime). Three sister decision shapes (FetchError + RetryPolicy from ADR 0033; submit/schedule/decode/drain phase boundary; chunk/proxy non-unification per deferred Slice 12) intentionally stayed as decisions rather than promoted to principles. Future cpu-cache ADRs should cite these where applicable.
