...
| 35 | Developer / Operator | As a developer or operator, I want server-authored dataset health so I can verify bindings, backend type, generated-coarse readiness, and source-cache state without reading logs. | CLI + Python | Open CPPX through the CLI, run `dataset health` in human and JSON modes, then call `workspace.datasets.health(...)` from Python. | Health reports binding, backend, generated coarse, cache counters, and status consistently across CLI and Python. | Pass | 2026-06-10 smoke used local server `http://127.0.0.1:9991`, workspace `b867894c-cafe-4439-8368-d8da2f65dc92`, and CPPX fixture. CLI human and JSON health reported `healthy`, `local`, ready binding, generated-coarse summary, and source-cache counters; Python printed `healthy local healthy`. |
