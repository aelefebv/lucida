...
`SavedViewVisibility` (`workspace.rs:92`), persisted as TEXT and serialized lowercase:
...
2. **Server authorizes as a reviewer** — `approve_saved_view` (`workspace.rs:3270`) uses `ensure_proposal_reviewable`, NOT the creator gate: `require_editor` → fetch → never-leak guard → must currently be `Proposed`. It then forbids self-approve (`created_by == reviewer` → `Forbidden`) and sets visibility `Shared`.
...
3. **Commit (window elapses)** — the timer fires `rejectSavedView(id)` → `rejectWorkspaceSavedView` (`POST …/reject`). `reject_saved_view` (`workspace.rs:3301`) — same reviewer authority as approve — sets visibility back to `Personal`. It is **non-destructive**: the saved camera and attribution are untouched; the proposer just keeps it privately.
