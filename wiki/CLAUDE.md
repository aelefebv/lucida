...
- **`systems/`** — major modules and runtime-architecture concepts, split into `crates/` (one article per current Cargo workspace member: `lucida-core`, `lucida-server`, `lucida-web`, `lucida-store`, `lucida-protocol`, `lucida-content`, `lucida-cli`, `lucida-py`; retired crate pages are explicitly historical) and `subsystems/` (web-internal modules and cross-cutting concepts: the chunk pipeline, planning, GPU residency, CPU cache, worker protocol, scene state and epochs, presence and follow, layouts, multichannel and colormaps, annotations, camera, auth, deployment).
