...
description: "Cross-cutting subsystem in lucida-server that gates every protected request through an AuthPrincipal extracted by middleware."
...
Saved-views (PRD #454) and any future feature consume `AuthPrincipal` without knowing about Google. Adding a new auth provider (Microsoft/Azure AD, Okta, generic OIDC) is a single-PR contribution per provider — see [Configurable From Day One for OSS Release](../../decisions/0017-configurable-from-day-one-for-oss-release.md).
...
The `lucida_session` cookie is opaque (256-bit random session ID) and carries no identity claims. Attributes per [Backend-Mediated OAuth with Session Cookies](../../decisions/0016-backend-mediated-oauth-with-session-cookies.md): `HttpOnly`, `Secure` auto-detected from request scheme, `SameSite=Lax`, `Path=/`, `Max-Age` matching the hard cap (default 30 days). `Lax` allows cross-origin top-level GET navigations (so saved-views link clicks work) while blocking cross-origin POST/PATCH/DELETE.

The cookie attribute set lives in one place (`auth/cookie.rs`) so logout's clearing cookie matches the original. `LUCIDA_COOKIE_SECURE={auto,always,never}` overrides the auto-detect for setups behind TLS-terminating proxies (see [OSS Config Defaults and the LUCIDA_* Env Var Contract](../../gotchas/oss-config-defaults.md)).

## Sign-in flow

End-to-end trace lives in [Flow: Authentication Sign-In](../../flows/auth-signin.md). Summary: unauthed browser → middleware returns inline `UnauthLanding` HTML with JS shim → shim captures `location.hash` and POSTs to `/auth/start` → server stashes intent in `pending_auth`, 302s to Google → user signs in → Google 302s to `/auth/callback` → server validates state token, exchanges code, validates JWT, applies hosted-domain check, creates `LoginSession`, sets cookie → 302 to original path with hash restored.
...
- **Auth checked at WS upgrade only.** Open WebSocket connections persist for their lifetime even if the underlying session expires. Mid-connection enforcement is a future feature ([Backend-Mediated OAuth with Session Cookies](../../decisions/0016-backend-mediated-oauth-with-session-cookies.md) §"Consequences").
...
- [Server-Stored Bookmarks and the AuthPrincipal Seam](../../decisions/0015-server-stored-bookmarks-and-auth-seam.md) — defines the `AuthPrincipal` contract this subsystem implements
- [Backend-Mediated OAuth with Session Cookies](../../decisions/0016-backend-mediated-oauth-with-session-cookies.md) — flow choice + cookie rationale
- [Configurable From Day One for OSS Release](../../decisions/0017-configurable-from-day-one-for-oss-release.md) — OSS posture for env-driven config
- [Auth Mode Auto-Detect by Bind Address](../../decisions/0018-auth-mode-auto-detect-by-bind-address.md) — bind-address safety model
- [Post-Logout Marker Cookie + `prompt=select_account`](../../decisions/0019-post-logout-marker-cookie-and-prompt-select-account.md) — `lucida_signed_out` marker cookie + `prompt=select_account` on the post-logout re-sign-in path
- [Flow: Authentication Sign-In](../../flows/auth-signin.md) — end-to-end sign-in trace
- [OSS Config Defaults and the LUCIDA_* Env Var Contract](../../gotchas/oss-config-defaults.md) — env-var contract and common misconfigurations
- [Deployment](deployment.md) — deploy-time perspective: OAuth provider setup, the `LUCIDA_COOKIE_SECURE=always` requirement behind TLS-terminating ingress, `PrincipalExtractor` extensibility for non-Google providers
- [lucida-server](../crates/lucida-server.md) — the crate this subsystem lives in
