...
- **Dev-mode boundary check**: `pipeline/planning/validate.ts` exports `validatePlanningInputs(snapshot, state)`, called from `plan()` gated by `import.meta.env.DEV`. Eight semantic-invariant checks throw a descriptive `Error` on first violation. (Originally nine — check 6, asset-catalog reference resolution, was withdrawn post-ship after a real-app trace showed the catalog is flattened across all datasets while the snapshot is per-dataset; the two legitimately diverge.) Vite dead-code-eliminates the call in production builds; the validator's helpers are fully tree-shakeable. Tests import the per-check helpers directly for per-invariant coverage.
