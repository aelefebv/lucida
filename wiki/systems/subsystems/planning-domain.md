...
description: "lucida-web/src/pipeline/planning/ — decides which chunks the renderer wants this tick."
...
`lucida-web/src/pipeline/planning/` decides which detail/coarse chunks the renderer wants this tick. Inputs come from WASM plus generated availability, minimap demand, and explicit carry-forward state; output is a `RequestPlan` consumed by [CPU Cache](cpu-cache.md) and the [GPU worker](gpu-residency.md). Pure modules own tier/LOD resolution, chunk enumeration/culling, emission/priority, validation, debug derivation, and top-level composition.
...
- **Consumers**: TickCoordinator stages every per-dataset result, reconciles workspace minimap priority, then atomically publishes the batch to [CPU Cache](cpu-cache.md) (`publishPlanningCycle`). The same staged plans produce full or delta cold-state messages for [gpu.worker.ts](gpu-residency.md); coordinator carry-forward advances only after the workspace plan succeeds.
...
- **`datasetId` is stamped at emit time, not post-hoc.** The snapshot carries it and the planner stamps every `ChunkRequest`; the tick coordinator does not mutate results.
...
- **Projected size is advisory.** Use `projected_diagonal_px` and importance for priority/debug reasoning; tier identity stays explicit.
