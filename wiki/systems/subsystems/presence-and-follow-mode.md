---
type: Subsystem
title: "Presence and Follow Mode"
description: "Presence is the per-client ephemeral state — viewport, cursor, follow target, dataset display settings — broadcast through lucida-server but never sequenced or persisted."
tags: [lucida, subsystem]
source_path: wiki/systems/subsystems/presence-and-follow-mode.md
created: 2026-04-18
modified: 2026-06-25
---
...
Presence is the per-client ephemeral state — viewport, cursor, follow target, dataset display settings — broadcast through [lucida-server](../crates/lucida-server.md) but never sequenced or persisted. Follow mode is **peer-to-peer**: any client can follow any other client; the server resolves transitive chains so that when A follows B and B follows C, A ends up effectively following C.
...
Peer-to-peer follow lets every client independently choose whom to follow (or no one). The server-side validation prevents loops and chains-of-followers, but otherwise stays out of the way. See [Peer-to-Peer Follow Mode](../../decisions/0002-peer-to-peer-follow-mode.md).

## Five wire messages

In `lucida-core/src/protocol.rs`:

- `ClientMessage::Presence { camera, view, display }` — viewport state. Throttled at ~50 ms in [`bridge.ts`](../crates/lucida-web.md).
...
Lives in `Session::set_follow` in [lucida-server](../crates/lucida-server.md). Two rules:
