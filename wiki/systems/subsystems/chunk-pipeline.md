...
3. **Render** — slice or volume path; throttled if only `residencyDirty` changed (≈30 fps cap on chunk-arrival redraws), immediate if `interactiveDirty`. Owned by [[gpu-residency]] downstream of `renderLoop.ts`. Post-PRD #622 the render side lives under `lucida-web/src/renderer/` as `coldState/`, `proxy/`, `volume/`, `slice/`, `worker/`, `descriptor/` subdirectories plus the pre-existing algorithmic core (`wantedSet.ts`, `proxyAtlas.ts`, `descriptorBuffer.ts`, …); `gpu.worker.ts` shrank from 815 LOC to a 34-LOC entry point. See [[decisions/0035-gpu-worker-split-into-renderer-subdirectories]].
