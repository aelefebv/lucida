...
- `migrations/` — versioned SQL migrations applied at startup (sqlx). Persistent state grew with [[auth]] (`login_sessions`, `pending_auth`), [[saved-views]] (`bookmarks` + `bookmark_datasets`), and workspaces (`workspaces`, `workspace_members`, `workspace_dataset_sources`, `workspace_datasets`, `workspace_saved_views`).
