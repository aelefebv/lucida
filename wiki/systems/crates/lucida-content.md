...
- `url.rs` — cross-platform dataset URL helpers per [[decisions/0042-canonical-dataset-url-form]]: three canonicalizing ops — `normalize_dataset_url` (idempotent string-level canonicalization), `is_local_dataset_url` (classifier over the canonical form), and `dataset_id_for_url` (BLAKE3 ID derivation). `dataset_url_hash16` shares the BLAKE3 digest with the ID (via a shared `blake3_url` helper) so they can't drift. [[lucida-core]]'s `saved_view` re-exposes the three public helpers as `#[wasm_bindgen]` shims so the SPA imports the same single source of truth.
