...
- `asset.rs` — legacy `AssetCatalog`, `ProxyAvailability`, `AssetCatalogDelta`. Re-exports `ProxyKind` (`GroupProxy3D`, `TileProxy3D`) via `pub use lucida_proxy::ProxyKind` (defined in lucida-proxy/src/spec.rs).
...
- **Wire format names are pinned via explicit `serde` tags**, not derived from `Debug`. `ProxyKind::GroupProxy3D` serializes as the literal string `"GroupProxy3D"`, asserted by the `proxy_kind_str` helper in the server. Renaming a variant requires touching both ends.
