...
- `backend.rs` — trusted library/client `open(url)` → `Arc<dyn ObjectStore>`. It normalizes at entry, then routes Unix `/path`, drive-letter `c:/path`, and UNC `//server/share/path` through cross-platform `LocalFileSystem`, plus `gs://`, `s3://`, and `http(s)://` remote stores. Server-local sources take the separate `ConfinedLocalRoot` → `AdmittedLocalDataset` path so descendant reads remain descriptor-confined and fail closed where that primitive is unavailable. Per [Canonical dataset URL form](../../decisions/0042-canonical-dataset-url-form.md) the normalization is idempotent and the canonical form is what every downstream consumer sees. Both cloud arms call `from_env()` to inherit each vendor's native env vars (`AWS_*` for S3; `GOOGLE_*` for GCS, including `GOOGLE_SERVICE_ACCOUNT*` and Google's standard `GOOGLE_APPLICATION_CREDENTIALS`). Picking `from_env()` over `new()` is the load-bearing choice — `new()` skips env discovery entirely. See [Use `GoogleCloudStorageBuilder::from_env()`, not `new()`, for GCS credentials](../../gotchas/gcs-credentials.md).
...
- `label_discovery.rs` — bounded collection-wide label-index discovery: exhaustive/sampled policy, anomaly expansion cap, deterministic probe scheduling, and retained label/color budgets. See [Collection Admission and Label Discovery](../subsystems/collection-admission-and-label-discovery.md).
...
- **Consumers**: [lucida-server](lucida-server.md) admits every source through `SourceTrustPolicy`; remote sources use `backend::open`, while local sources keep an `AdmittedLocalDataset` capability. It calls `import::import_dataset` once on open and `cache::CachedStore` for every chunk read. [lucida-py](lucida-py.md) uses the trusted `backend::open` path through `PyStore`.
...
- **Every declared collection tile crosses the same strict admission boundary.** Metadata reads are bounded to 32 concurrent operations, but results are assembled and diagnosed in declaration order. No representative tile lends geometry to an unreadable or structurally different neighbor.
- **Label discovery and label retention have separate budgets.** Wide collections may sample label indexes, while any discovered labels are built sequentially against one dataset-wide label/color budget. Set `LUCIDA_EXHAUSTIVE_LABEL_DISCOVERY=1` when completeness is more important than avoiding one metadata request per tile.
...
`chunk_shape` parallels `ImageBindingSeed.axes_names` (one entry per on-disk axis) and is consumed by the resolver to translate wire `t/c` voxel coords into disk-grid coords. `chunk_byte_layout` carries the precomputed strides + chunk sizes used by the slice step. The live consumers (`ChunkResolver::level_info(image_id, level)`, source chunk serving, and generated-coarse source-volume reads) take the level index and read all fields off one record.
