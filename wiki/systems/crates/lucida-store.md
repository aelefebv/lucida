...
- **Explicitly-positioned collections have translations in physical units** in OME-Zarr, but lucida composes transforms in voxel units. The import converts using the level-0 X/Y scale before forming the `tile → group` transform. See [Explicit Translations Are in Physical Units; Lucida Composes in Voxels](../../gotchas/stage-translations-are-microns.md).
