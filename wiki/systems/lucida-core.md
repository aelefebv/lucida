...
- **`Scene::apply` is the only path that mutates a Scene.** Every command — document or viewport — flows through it. Don't poke fields directly; doing so bypasses epoch bumps and derived-state rebuilds (see [[scene-state-and-epochs]]).
