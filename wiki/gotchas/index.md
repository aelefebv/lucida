...
- [At Extreme Tile Counts the Per-Entity Descriptor Buffer Is the Ceiling](descriptor-buffer-extreme-tile-count.md) — at ~21k+ visible tiles the render worker can throw `Array buffer allocation failed` in headless/low-memory browsers; the driver is the O(members×channels) descriptor buffer bound whole as a STORAGE buffer (real ceiling `maxStorageBufferBindingSize` ~155k entities), not the bounded 64/512 MB atlas texture budgets — capping visible tiles is a false fix
