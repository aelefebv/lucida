...
> Note (since): the top placement is conditional on the seed set actually being small — the bounded-starvation premise in "Why" (~16 chunks, one-to-two seconds). A wide collection's whole-collection seed set is tens of thousands of coarsest chunks; at top priority that holds every fetch slot for tens of minutes while the visible band waits. Above `MINIMAP_SEED_FAST_MAX_CHUNKS` pending chunks, `applyWorkspaceMinimapPriority` floors every bulk seed at `max(MINIMAP_SEED_BULK_LANE_OFFSET = 2600, highest view-serving priority anywhere in the workspace + 1)`. The workspace reconciliation matters: plans are produced per dataset but share one fetch scheduler, and another dataset's unbounded distance term can exceed this dataset's local floor. The minimap then fills opportunistically as fair slots free up. Small seed sets keep the top lane unchanged.
...
- `lucida-web/src/pipeline/planning/plan.ts::applyWorkspaceMinimapPriority` — reconciles bulk floors across all per-dataset plans before their one-shot cache publication.
