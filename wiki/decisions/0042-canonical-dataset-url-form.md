...
- **Cloud URLs** (`gs://`, `s3://`, `http://`, `https://`) — passthrough, except that a leading URI scheme is lowercased (`HTTP://Host/Path` → `http://Host/Path`). Schemes are case-insensitive per RFC 3986 §3.1, so the lowercase spelling is the canonical one; everything after `://` — host, bucket, object path — is byte-for-byte untouched because bucket and object names are case-sensitive.
