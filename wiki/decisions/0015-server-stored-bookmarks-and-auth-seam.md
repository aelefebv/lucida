...
- **Persistence with names.** A URL hash carries no name, no creator, no created-at; it's an opaque blob. For curated analyses ("Group B7 analysis"), a named entry is qualitatively different.
...
- **Pre-auth bookmarks** carry `created_by: "dev@local"`. Migration policy on auth cutover is open (Queue — Open Questions).
...
- `lucida-server/src/bookmarks/handlers.rs` — REST handlers under `/api/bookmarks`. POST overwrites `created_by` from `AuthPrincipal` (request body cannot spoof). PATCH/DELETE check `bookmark.created_by == principal.email || principal.is_admin`. Hand-rolled `parse_dataset_params` against `RawQuery` for repeated `?dataset=…` (Axum's default `Query<T>` drops repeats; see Axum's Default Query Extractor Drops Repeated Keys).
...
- Queue — Open Questions — auth-cutover migration question; selected-dataset wrinkle
