/**
 * Tests for `synthesizeGroupRosterEntry`.
 *
 * Pure-function tests — no orchestrator state.
 *
 * Contract under test:
 *   - Union the 3D AABB of every child tile's `[0,1]^3` cube transformed
 *     by its `member_model_matrix`.
 *   - Union the 2D AABB of every child tile's `(layoutPositionVox, lvl0 X/Y)`.
 *   - Return `null` if either AABB is degenerate (zero span on any axis,
 *     no contributing tiles, or all matrices were the wrong length).
...
import { synthesizeGroupRosterEntry } from "./roster.ts";
 ...
 * supplied matrices keyed by `imageId`. Other ctx tiles are unused by
 * `synthesizeGroupRosterEntry` and are stubbed as `unknown`.
 */
function makeCtx(
...
/**
 * Construct a `TileSnapshot`-shaped object with the minimum surface
 * the helper reads: `imageId`, `layoutPositionVox`, and `levels[0].shape`.
 * Other tiles are filled with defensive defaults so the type-erased
...
describe("synthesizeGroupRosterEntry", () => {
  it("unions 3D AABB and 2D footprint across two visible tiles", () => {
    // Tile A: 3D cube [0..10] on each axis; 2D footprint origin (0,0) sized 100x100.
    // Tile B: 3D cube [20..30] on each axis; 2D footprint origin (100,100) sized 100x100.
    ...
    const tiles: EntitySnapshot[] = [
      makeTile("img-a", [0, 0],     [1, 1, 1, 100, 100]),
      makeTile("img-b", [100, 100], [1, 1, 1, 100, 100]),
    ];

    const result = synthesizeGroupRosterEntry(ctx, "ds1", "group-0", tiles);

    expect(result).not.toBeNull();
    const r = result!;
    expect(r.entityId).toBe("group-0");
    expect(r.imageId).toBe("group-0");
    expect(r.mode).toBe("group-as-proxy");
  ...
  it("matches the single tile's AABB when only one tile is visible", () => {
    // Single tile at world translate (5,7,9), scale (4,4,4); 2D origin (10,20).
    ...
    const tiles: EntitySnapshot[] = [
      makeTile("img-only", [10, 20], [1, 1, 1, 30, 40]),
    ];
  ...
  it("returns null when no tiles are passed (zero visible tiles)", () => {
    const ctx = makeCtx({});
    const result = synthesizeGroupRosterEntry(ctx, "ds1", "group-0", []);
    expect(result).toBeNull();
  });

  it("returns null when the tile's matrix yields a degenerate (zero-span) AABB", () => {
    ...
    const tiles: EntitySnapshot[] = [
      makeTile("img-degen", [0, 0], [1, 1, 1, 100, 100]),
    ];
  ...
  it("skips tiles whose model matrix has length !== 16", () => {
    // Tile A returns a length-9 matrix → skipped (no 3D corners contributed).
    // Tile B returns a valid 16-length matrix → drives the 3D AABB alone.
    const ctx = makeCtx({
      "img-bad":  new Float32Array(9),                              // wrong length
      "img-good": translateScaleMatrix(0, 0, 0, 10, 10, 10),        // valid
    });
    const tiles: EntitySnapshot[] = [
      makeTile("img-bad",  [0, 0],     [1, 1, 1, 100, 100]),
      makeTile("img-good", [200, 200], [1, 1, 1, 100, 100]),
    ];
    ...
    expect(r.modelMatrix![12]).toBe(0);
    // 2D union covers both tiles → origin (0,0), span (300, 300).
    expect(r.position).toEqual([0, 0]);
