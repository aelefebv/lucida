...
describe("InteractionModeDetector", () => {
  describe("adaptive eviction", () => {
    it("detects panning from viewEpoch velocity", () => {
      ...
      for (let i = 0; i < 5; i++) {
        detector.push(makeSceneEpochs({ view: i + 1, selection: 1 }));
      }
    ...
    it("detects scrubbing from selectionEpoch velocity", () => {
      ...
      for (let i = 0; i < 5; i++) {
        detector.push(makeSceneEpochs({ view: 1, selection: i + 1 }));
      }
    ...
    it("reports idle when no epochs bumping", () => {
      ...
      for (let i = 0; i < 5; i++) {
        detector.push(makeSceneEpochs({ view: 1, selection: 1 }));
      }
    ...
    it("scrubbing mode protects prefetch over demoted", () => {
      ...
      detector.push(makeSceneEpochs({ view: 1, selection: 6 }));
      detector.push(makeSceneEpochs({ view: 1, selection: 7 }));
      detector.push(makeSceneEpochs({ view: 1, selection: 8 }));
