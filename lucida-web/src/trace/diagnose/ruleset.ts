 ...
 *
 * 1. **Absolute p95 ceilings** for I/O and compute stages, each set above the
 *    worst healthy p95 observed rather than at it.
 * 2. **Backlog ETA** for queue stages, which get no per-chunk ceiling at all.
...
import type { StageClass } from "./types.ts";
...
export interface AbsoluteRule {
  id: string;
  /** The stage this ceiling judges. One rule, one stage: a shared ceiling hides which stage it was set for. */
  stage: string;
  stat: "p95";
  ceilMs: number;
  why: string;
...
export interface Ruleset {
  ...
  /** Queue stages, listed so their *absence* from `absolute` reads as deliberate. */
  queueStages: readonly string[];
  backlog: BacklogRule;
...
/**
 * Which threshold family may judge a stage. Declared next to the stage
 * inventory rather than inferred from a name, so adding a stage forces the
 * decision instead of defaulting to one.
 */
export const STAGE_CLASSES: Record<string, StageClass> = {
...
export const RULESET: Ruleset = {
  ...
  absolute: ABSOLUTE,
  queueStages: Object.entries(STAGE_CLASSES)
