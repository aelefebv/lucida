...
describe("observation only", () => {
  it("offers no control that could change what the pipeline does", () => {
    ...
    // Every button in the dock reads, saves, drills in, or moves the dock. If
    // a future change adds one that does not, this list is where it shows up.
    const labels = screen.getAllByRole("button").map((node) => node.textContent);
    for (const label of labels) {
      expect(label).toMatch(
        /^(Close|Pop out|Read the newest run|Save run|Save for Perfetto|Save bundle|Show the rows behind .*|Close drill-down)$/,
      );
    }
  });
