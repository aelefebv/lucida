...
// read from the LUT (`< 65536`) or computed here (`>= 65536`). Byte-identical to
// the copy in slice.wgsl.
//
  ...
  if (entity.isLabel == 1u) {
    ...
    for (var i = 0; i < labelMaxSteps; i++) {
      ...
      if (lval != 0xFFFFFFFFu && lval != 0u) {
        ...
        // truncation): the 256×256 LUT holds the flat 65536-entry table
        // row-major (entry `idx` at (idx & 255, idx >> 8), no sampler, explicit
        // `image-label.colors` over a glasbey fill) for `lval < 65536`; ids the
        // LUT cannot hold (`>= 65536`) get the SAME glasbey colour computed
        // in-shader. Previously this masked the id to its low 16 bits,
        // collapsing e.g. 70000→4464 and any multiple of 65536→0 (transparent)
        // — the fixed defect.
        var rgb: vec3<f32>;
        if (lval < 65536u) {
          rgb = textureLoad(labelLutTex, vec2i(i32(lval & 255u), i32(lval >> 8u)), 0).rgb;
        } else {
          rgb = glasbey_wgsl(lval);
        }
