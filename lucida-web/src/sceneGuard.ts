 ...
 *
 * Observers are module-global because the scene itself is process-global:
 * a fatal engine error poisons every caller equally, no matter which module
 * issued the call that tripped it.
 */
 ...
 *   undefined state, and the bindings' borrow poisoning ("recursive use of
 *   an object …") persists across calls once tripped. Every subsequent scene
 *   call fails while the JS UI stays healthy.
...
export interface SceneCallObserver {
  /** A guarded scene mutation completed successfully. */
  onSceneCallApplied(context: string): void;
  ...
  onSceneCallFailed(error: unknown, context: string): void;
}
...
/**
 * Run one scene mutation and report its outcome. Transparent to the caller:
 * returns the call's value, rethrows the call's error. Observer failures are
 * contained (logged, never propagated) so a misbehaving observer cannot
 * corrupt an otherwise healthy apply path.
 */
export function guardedSceneCall<T>(context: string, call: () => T): T {
  let value: T;
  try {
    value = call();
  } catch (e) {
    for (const observer of [...observers]) {
      try {
        observer.onSceneCallFailed(e, context);
      } catch (observerErr) {
  ...
  for (const observer of [...observers]) {
    try {
      observer.onSceneCallApplied(context);
    } catch (observerErr) {
