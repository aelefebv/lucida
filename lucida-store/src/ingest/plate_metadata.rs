...
mod tests {
    ...
    #[tokio::test]
    async fn round_trip_plate_metadata() {
        let dir = temp_dir("round_trip");

        // Build a PlateLayout with 2 wells, 4 FOVs each.
        let layout = make_layout(
            "round_trip_plate",
            vec!["A", "B"],
            vec!["1", "2"],
        ...
        // Write well metadata and FOV fixtures for each well.
        for well in &layout.wells {
            write_well_metadata(&dir, well).unwrap();

            for (i, _fov) in well.fovs.iter().enumerate() {
                let fov_dir = dir
                    .join(&well.row_name)
                    .join(&well.col_name)
                    .join(i.to_string());
                write_fov_fixture(&fov_dir);
            }
        }

        // Read back using read_plate_info.
        let store = crate::backend::open(dir.to_str().unwrap()).unwrap();
        let plate = crate::metadata::read_plate_info(&store, "round_trip").await.unwrap();

        // Verify round-trip: rows.
        assert_eq!(plate.rows, vec!["A", "B"]);

        // Verify round-trip: columns.
        assert_eq!(plate.columns, vec!["1", "2"]);

        // Verify round-trip: wells.
        ...
        assert_eq!(plate.wells[1].path, "B/2");
        assert_eq!(plate.wells[1].row_index, 1);
        assert_eq!(plate.wells[1].column_index, 1);
        assert_eq!(plate.wells[1].fovs.len(), 4);

        // Verify round-trip: plate name.
        assert_eq!(plate.name, "round_trip_plate");

        // Verify uniform FOV metadata was parsed.
        assert_eq!(plate.num_levels, 1);
        assert_eq!(plate.fov_shape, [1, 256, 256]);
