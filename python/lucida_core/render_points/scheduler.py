"""Coalescing invalidation scheduler for per-view points frame planning."""
...
class InvalidationTicketPoints:
    plan_seq: int
    kind: InvalidationKind
    reasons: list[str]


class RenderPointsInvalidationScheduler:
    """Track and coalesce invalidations to one points ticket per view."""
    ...
    def consume(self, *, session_id: str, view_id: str) -> InvalidationTicketPoints | None:
        key = (session_id, view_id)
        ...
        return InvalidationTicketPoints(
            plan_seq=seq,
            kind=top_kind,
