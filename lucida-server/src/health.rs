//! Liveness and readiness probes for containerized deploys.
//!
...
//! route requests here.
//!
//! Both routes return `200 OK` with body `"ok"` today. The split is
//! intentional even though they behave identically: future drain-on-
//! shutdown semantics will flip readiness to 503 while liveness stays
//! 200, so the LB stops routing but the kubelet doesn't restart the
//! pod mid-drain. The boundary belongs in code now so the manifests
//! (and any future drain logic) have stable URLs to point at.
//!
//! ## Auth interaction
//!
//! Both probes MUST land on the public router half — the kubelet does
//! not present a session cookie, and probes timing out due to a 401
...
/// Build the health-probe router with `/healthz` and `/readyz` mounted.
///
...
/// that a kubelet without credentials can hit the probes.
