...
//! is the pool type and the row type, and nothing else, because both
//! payload columns are text on either engine.
...
fn row_to_entry(row: &sqlx::sqlite::SqliteRow) -> InboxEntry {
    InboxEntry {
        id: row.get("id"),
