...
9. **Rendering** — shader dispatch, compositing, semantic fallback chain. Unblocked by PRD #432 (descriptor buffers landed); remaining work: unified `target detail LOD → coarser detail LODs → overview proxy → empty` fallback chain in slice/volume shaders.
