...
8. **GPU Residency** — atlas, page table, descriptors, wanted-set reporting. *(done — persistent atlas, multi-LOD indirection, shader fallback, cold-state-driven management, see PRD #383; shared atlas pools per (dataset, channel, chunk dims) for plate FPS, see PRD #393; per-entity descriptor buffers + direct proxy handle, see PRD #432 — model matrix, channel mask, proxy handle, page table base, per-LOD chunk dims, contrast/gamma/opacity/colormap all moved into per-dataset EntityDescriptor[] storage buffer; rayHitLocal extracted into a dedicated viewEpoch hot-state message)* Deferred from initial implementation: temporal runway management (adjacent-T pages physically resident but unmapped — currently handled by natural eviction).
9. **Rendering** — shader dispatch, compositing, semantic fallback chain. *(done — unified `target detail LOD → coarser detail LODs → field proxy → well proxy → empty` chain in slice/volume shaders, see PRD #436. Per-draw `renderMode` flag removed; descriptor sentinels drive availability detection. Handlers detect "no detail" via `entityLodMetas` instead of a mode field.)*
