'use strict';
// Trace-volume driver (issue #888). Drives the real lucida SPA at DPR2 against an
// already-opened dataset and dumps window.__tv event counters per phase:
...
(async () => {
  ...
  const result = { ok: false, url, dpr, phases: {}, ready: {}, heap: {}, net: {} };
  try {
    ...
    // ---- DEBUG PANEL OPEN: same pan, with the existing gauges observed ---------
    try {
      await page.locator('button', { hasText: /^Debug$/ }).first().click({ timeout: 5000 });
      await page.waitForTimeout(1500);
      result.debug_panel_opened = true;
    } catch (e) { result.debug_panel_opened = 'click_failed: ' + String(e).split('\n')[0]; }
    await page.evaluate(reset);
    await pan(page, panMs, result.targets = result.targets || {}, 'pan_debug_panel');
    result.phases.pan_debug_panel = await page.evaluate(dump);
    result.heap.after_pan_debug_panel = await page.evaluate(heap);
    await page.screenshot({ path: outDir + '/pan-debug-panel.png' });
    try {
      await page.locator('button', { hasText: /^Debug$/ }).first().click({ timeout: 5000 });
    ...
    // ---- 3D VOLUME MODE --------------------------------------------------------
    try {
      await page.locator('button', { hasText: /^3D$/ }).first().click({ timeout: 5000 });
      let p3 = null; const dl3 = Date.now() + 60000;
      while (Date.now() < dl3) { p3 = await page.evaluate(readyProbe); if (p3 && p3.ready) break; await page.waitForTimeout(250); }
      await page.waitForTimeout(4000);
      result.ready.volume3d = p3;
      await page.evaluate(reset);
      await page.waitForTimeout(4000);
      result.phases.volume3d_settle = await page.evaluate(dump);
      await page.evaluate(reset);
      await pan(page, panMs, result.targets = result.targets || {}, 'volume3d_orbit');
      result.phases.volume3d_orbit = await page.evaluate(dump);
      await page.screenshot({ path: outDir + '/volume3d.png' });
      result.heap.after_volume3d = await page.evaluate(heap);
    } catch (e) { result.volume3d_error = String(e).split('\n')[0]; }
    ...
    fs.writeFileSync(outDir + '/console.log', messages.join('\n'));
    fs.writeFileSync(outDir + '/tv-summary.json', JSON.stringify(result, null, 2));
    result.ok = true;
    out({ ok: true, out_dir: outDir, wrote: 'tv-summary.json', ready: result.ready });
  } catch (e) {
    try { fs.writeFileSync(outDir + '/console.log', messages.join('\n')); } catch (_) {}
    try { fs.writeFileSync(outDir + '/tv-summary.json', JSON.stringify(result, null, 2)); } catch (_) {}
