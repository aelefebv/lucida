...
class RealSpaMatrixTests(unittest.TestCase):
    ...
    def test_layers_dialog_focus_wait_uses_the_product_dialog_identity(self) -> None:
        self.assertIn("async function installBoundedFocusTrace", web_surface._SPA_DRIVER)
        self.assertIn("Reflect.apply(nativeFocus, target, args)", web_surface._SPA_DRIVER)
        self.assertIn("firstEntryLimit = 128", web_surface._SPA_DRIVER)
        self.assertIn("tailEntryLimit = 384", web_surface._SPA_DRIVER)
        self.assertIn("dropped_count: droppedCount", web_surface._SPA_DRIVER)
        self.assertIn("before-failure-screenshot", web_surface._SPA_DRIVER)
        self.assertIn("after-cycle-first-focus", web_surface._SPA_DRIVER)
        self.assertIn("drawer_focus_trace: drawerFocusTrace", web_surface._SPA_DRIVER)
        self.assertIn("async function waitForFocusInside", web_surface._SPA_DRIVER)
        self.assertIn("expected_focus_state: expectedState", web_surface._SPA_DRIVER)
        self.assertIn("document_has_focus: document.hasFocus()", web_surface._SPA_DRIVER)
    ...
    def test_focus_trace_preserves_native_semantics_and_bounded_cleanup(self) -> None:
        helper_start = web_surface._SPA_DRIVER.index(
            "async function installBoundedFocusTrace",
        )
...
class FakeElement {
  ...
  getAttribute(name) {
    if (name === 'aria-label') return this.id;
    return null;
  }
}
...
(async () => {
  ...
  const target = new HTMLElement('close');
  const installed = await installBoundedFocusTrace(page);
  const firstTrace = window.__lucidaTryoutFocusTrace;
  const returned = target.focus();
  await Promise.resolve();
  const stopped = await stopBoundedFocusTrace(page, 'return-complete');
  const stoppedAgain = firstTrace.stop();
  const restoredAfterReturn = HTMLElement.prototype.focus === original;

  const sentinel = new Error('native sentinel');
  const throwingNative = function() { throw sentinel; };
  HTMLElement.prototype.focus = throwingNative;
  const throwingInstall = await installBoundedFocusTrace(page);
  let caught = null;
  try {
    target.focus();
  } catch (error) {
  ...
  const throwingTrace = window.__lucidaTryoutFocusTrace;
  const throwingStopped = throwingTrace.stop();
  const restoredAfterThrow = HTMLElement.prototype.focus === throwingNative;

  HTMLElement.prototype.focus = original;
  const boundedInstall = await installBoundedFocusTrace(page);
  for (let index = 0; index < 300; index++) target.focus();
  await Promise.resolve();
  const bounded = await stopBoundedFocusTrace(page, 'bounded-complete');
  const lastEntry = bounded.entries[bounded.entries.length - 1];
  const listenerCount = Array.from(listeners.values())
    .reduce((total, registered) => total + registered.size, 0);

  process.stdout.write(JSON.stringify({
    installed: installed.installed,
    returned,
    stoppedInstalled: stopped.installed,
    stoppedAgainInstalled: stoppedAgain.installed,
    restoredAfterReturn,
    throwingInstalled: throwingInstall.installed,
    caughtSame: caught === sentinel,
    restoredAfterThrow,
    throwingError: throwingStopped.entries.find((entry) => entry.kind === 'focus-call').error,
    boundedInstalled: boundedInstall.installed,
    boundedLength: bounded.entries.length,
    boundedTruncated: bounded.truncated,
    boundedDropped: bounded.dropped_count,
    finalMarker: lastEntry && lastEntry.label,
    listenerCount,
    restoredAtEnd: HTMLElement.prototype.focus === original,
  }));
})().catch((error) => {
  console.error(error);
  process.exit(1);
...
"""
        ...
        self.assertEqual(receipt["returned"], "native-result")
        self.assertTrue(receipt["stoppedInstalled"])
        self.assertTrue(receipt["stoppedAgainInstalled"])
        self.assertTrue(receipt["restoredAfterReturn"])
        self.assertTrue(receipt["throwingInstalled"])
        self.assertTrue(receipt["caughtSame"])
        self.assertTrue(receipt["restoredAfterThrow"])
        self.assertEqual(receipt["throwingError"], "native sentinel")
        self.assertTrue(receipt["boundedInstalled"])
        self.assertEqual(receipt["boundedLength"], 512)
        self.assertTrue(receipt["boundedTruncated"])
        self.assertGreater(receipt["boundedDropped"], 0)
        self.assertEqual(receipt["finalMarker"], "bounded-complete")
        self.assertEqual(receipt["listenerCount"], 0)
        self.assertTrue(receipt["restoredAtEnd"])
