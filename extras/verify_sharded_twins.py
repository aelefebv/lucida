...
# ///
"""Prove reading a sharded dataset end to end, at device pixel ratio 2.
...
1. Every run reached quiescence. The page published quiescent and closed
   its own run.
2. Each pair's two frames are the size the viewport makes at device pixel
   ratio 2, are not blank, and are identical pixel for pixel, so how the
   objects are laid out never changes what the viewer shows. Every run pins
   the contrast window (``lucida trace --pin-contrast``). The default fit
   samples whatever is resident when it runs, and two runs of one dataset
   then draw the same data a level or two apart (issue #1037). The report
   names the largest difference it saw.
3. In the sharded runs, every backend read the trace recorded waited on the
   chunk read cap and moved exactly the inner chunks it carried, one shard
   index, or both. A read may carry one inner chunk, or a byte-contiguous
   stretch of one shard's inner chunks merged into one read. The expected
   sizes come from the shard indexes on disk, so the comparison is exact
   rather than a threshold. The check also asserts that the view wanted no
   shard whole, so a shard downloaded whole is never a size the audit
   accepts.
...
5. The second pair has one source level too large to serve as the coarse
   ...
   and receive that generated level, and the pair still matches. With
   ``--generated-cache-dir``, the check also compares the two generated
   tiers byte for byte on the server's disk.

The second pair is one image whose source grid holds fewer than the 32
chunks the server generates on its own after an open. A generated level has
one chunk per source chunk, and a static view does not ask for the rest
(issue #1034). The check opens each of the pair's datasets first and waits
for that fill to finish, so the run measures a complete tier and not the
fill. The fill reads the whole source through the object store, so the
server answers the view's own requests from its cache and this pair's read
audit meets no reads. The read audit is the first pair's job. This pair's
job is the picture and, with the cache directory, the generated bytes.
...
Options:
    ...
    --shard S            Shard edge, both pairs. Default 1024. Keep it larger
                         than what the view wants whole: a shard the view
                         wants every inner chunk of is one the audit cannot
                         tell from a shard downloaded whole.
    ...
    --generated-cache-dir DIR
                         The server's generated coarse cache root (its
                         LUCIDA_GENERATED_COARSE_CACHE_DIR). When given, the
                         generated tiers of the second pair are compared
                         byte for byte.
...
# The axes of a chunk key, `level/t/c/z/y/x`, after the level.
...
# The driver's default too, but named here so a change to the default cannot
...
# How long to wait for the server's generated fill after an open, and how many
# one-second readings of its ready count must agree before the fill counts as
# finished. A fill of under 32 chunks takes well under a second, so a plateau
# this long mid-fill is a stall, and the run reports it.
GENERATED_FILL_TIMEOUT_SECONDS = 60
GENERATED_FILL_STEADY_READINGS = 3


# ---------------------------------------------------------------------------
# What the object store holds on disk
# ---------------------------------------------------------------------------
...
def image_prefix(image_id: str) -> str:
    """The object prefix of an image, from the id the trace rows carry.
...
def grid_coords(chunk_key: str, layout: ArrayLayout) -> tuple[str, tuple[int, ...]]:
    ...
    bundled several to a chunk is divided by that chunk size, as the server
    does.
    """
...
def read_shard_index(shard_path: Path, layout: ArrayLayout) -> list[tuple[int, int]]:
    ...
    Parsed here rather than through zarr so that the expected sizes are an
    independent reading of the object.
    """
...
@dataclass(frozen=True)
class ReadCost:
    """What one chunk key costs to read from this object store, in bytes.
    ...
    def allowed(self, wanted: Iterable[int] | None = None) -> dict[int, str]:
        ...
        if not self.written:
            return allowed
        stretches = contiguous_stretches_through(self.entries, self.position, set(wanted or ()) | {self.position})
        for count, stretch_bytes in stretches:
            kind = "inner-chunk" if count == 1 else "inner-chunks-merged"
            allowed.setdefault(stretch_bytes, kind)
            allowed.setdefault(stretch_bytes + self.index_bytes, f"index+{kind}")
        return allowed


def contiguous_stretches_through(
    entries: Sequence[tuple[int, int]], position: int, wanted: set[int] | None = None
) -> list[tuple[int, int]]:
    """Every stretch of entries contiguous in the object that includes ``position``.

    Returns ``(count, bytes)`` per stretch. Entries are laid out in the order
    they were written, which need not be grid order, so stretches follow
    byte offsets rather than positions. A stretch holds only written
    entries, and only positions in ``wanted`` when that is given.
    """
    written = sorted(
        (offset, nbytes, index)
    ...
    for start in range(ordinal, -1, -1):
        ...
        for end in range(ordinal, len(written)):
            if end > ordinal and written[end - 1][0] + written[end - 1][1] != written[end][0]:
                break
            stretches.append((end - start + 1, sum(nbytes for _, nbytes, _ in written[start : end + 1])))
    return stretches
...
def read_cost(root: Path, image_id: str, chunk_key: str) -> ReadCost | None:
    """The bytes a read of ``chunk_key`` should move, or None for a generated level."""
    located = locate_chunk(root, image_id, chunk_key)
    if located is None:
        return None
    layout, shard_path, position = located
    if not layout.sharded:
        return ReadCost(path=shard_path, object_bytes=shard_path.stat().st_size if shard_path.is_file() else 0)
    if not shard_path.is_file():
        return ReadCost(path=shard_path, index_bytes=layout.index_bytes, written=False)
...
class ReadAudit:
    ...
    # Shards the run wanted every inner chunk of. Their whole length is a
    # legitimate merged read the audit cannot tell from a shard downloaded
    # whole, so the check requires zero.
    shards_wanted_whole: int = 0
...
def audit_reads(root: Path, run: dict) -> ReadAudit:
    ...
    # Every position of every shard the run asked for, which are the only
    # neighbours a merged read may carry.
    wanted: dict[Path, set[int]] = {}
    capacity: dict[Path, int] = {}
    for keys in keys_by_label.values():
        for image_id, chunk_key in keys:
            located = locate_chunk(root, image_id, chunk_key)
            if located is None:
                continue
            layout, path, position = located
            wanted.setdefault(path, set()).add(position)
            if layout.sharded:
                capacity[path] = math.prod(layout.chunks_per_shard)
    audit.shards_wanted_whole = sum(1 for path, total in capacity.items() if len(wanted[path]) == total)
    ...
    for row in run.get("serverRows", []):
        ...
        if not keys:
            audit.unlabelled += 1
            continue
        costs = {key: read_cost(root, *key) for key in sorted(keys)}
        if all(cost is None for cost in costs.values()):
            audit.generated += 1
        ...
        if bytes_moved is None:
            audit.no_read += 1
            continue
        if "permit-wait" not in row.get("phases", {}):
            audit.violations.append(f"label {label}: read {bytes_moved} bytes without waiting on the chunk read cap")
            continue
        ...
        for cost in costs.values():
            if cost is None:
                continue
            if cost.sharded and cost.path.is_file():
                shard_bytes = cost.path.stat().st_size
                audit.smallest_shard = (
                    shard_bytes if audit.smallest_shard is None else min(audit.smallest_shard, shard_bytes)
                )
            kind = cost.allowed(wanted.get(cost.path)).get(bytes_moved)
            if kind is not None:
                matched = kind
                break
        if matched is None:
            expected = "; ".join(
                f"{image_id} {chunk_key}: {sorted(cost.allowed(wanted.get(cost.path)).items())}"
                for (image_id, chunk_key), cost in costs.items()
                if cost is not None
            )
...
def compare_frames(first: Path, second: Path, diff_out: Path | None = None) -> FrameComparison:
    ...
    ``diff_out`` gets a picture of the pixels that differ, and nothing when
    there are none.
    """
    a = load_frame(first)
    b = load_frame(second)
    ...
    if differing.any() and diff_out is not None:
...
class DrivenRun:
    ...
    def dataset_ids(self) -> list[str]:
        return list(self.run["header"].get("datasetIds", []))
...
class Lucida:
    ...
    def generated_ready_chunks(self, workspace_id: str, timeout: float = 60.0) -> int:
        return ready_generated_chunks(self.call("--workspace", workspace_id, "dataset", "health", timeout=timeout))
...
def wait_for_generated_fill(lucida: Lucida, workspace_id: str, timeout_seconds: float) -> int:
    ...
    then stops, so the fill is done when its ready count holds still for
    ``GENERATED_FILL_STEADY_READINGS`` readings a second apart and at least
    one chunk is ready. A fill that has not started by the deadline is
    reported as what it is: zero.
    """
    ...
    while True:
        ...
        steady = readings[-GENERATED_FILL_STEADY_READINGS:]
        if ready > 0 and len(steady) == GENERATED_FILL_STEADY_READINGS and len(set(steady)) == 1:
            return ready
...
def generated_tier(cache_dir: Path, dataset_id: str) -> dict[str, str]:
    """Every generated chunk the server holds for ``dataset_id``, as a content hash by level and key.

    The server keeps them under ``<cache_dir>/<source>/<identity>/<dataset
    id>/L<level>/<key>.bin``; the identity differs between the twins, so
    the chunks are matched on the level and key alone.
    """
    return {
        f"{chunk.parent.name}/{chunk.name}": hashlib.sha256(chunk.read_bytes()).hexdigest()
        for chunk in cache_dir.glob(f"*/generated-coarse-*/{dataset_id}/L*/*.bin")
    }
...
def check_pair(pair: TwinPair, unsharded: DrivenRun, sharded: DrivenRun, out: Path, report: Report) -> None:
    for run in (unsharded, sharded):
        report.check(
            run.quiescent,
            f"{pair.name}: {run.dataset.name} reached quiescence "
            f"(end reason {run.end_reason}, verdict {run.verdict})",
        )

    diff = out / f"{pair.name}-diff.png"
    frames = compare_frames(unsharded.screenshot, sharded.screenshot, diff)
    expected_size = tuple(side * DEVICE_PIXEL_RATIO for side in unsharded.viewport)
    report.check(
        (frames.width, frames.height) == expected_size,
        f"{pair.name}: frames are {frames.width}x{frames.height}, the viewport at device pixel ratio "
        f"{DEVICE_PIXEL_RATIO}",
    )
    report.check(
        not frames.blank,
        f"{pair.name}: both frames are not blank ({frames.colors[0]} and {frames.colors[1]} distinct colors)",
    )
    report.check(
        frames.identical,
        f"{pair.name}: frames identical "
        f"(largest difference {frames.max_delta}, {frames.differing_fraction:.2%} of pixels differ)"
        + ("" if frames.identical else f"; see {diff}"),
    )

    audits = {run.dataset: audit_reads(run.dataset, run.run) for run in (unsharded, sharded)}
    for run in (unsharded, sharded):
        audit = audits[run.dataset]
        ...
        # An audit that met no reads proves nothing, because a run answered
        # from the server's source cache looks perfect. The generated-coarse
        # pair is exempt because its fill read the whole source before the
        # view opened. It proves the picture, and the read audit is the
        # other pair's job.
        ...
        report.check(
            audit.ok,
            f"{pair.name}: every backend read in {run.dataset.name} waited on the chunk read cap "
            "and is sized to the keys it carried",
        )
    if not pair.generated_coarse:
        sharded_audit = audits[sharded.dataset]
        report.check(
            sharded_audit.range_reads > 0,
            f"{pair.name}: the sharded run read inner chunks by range ({sharded_audit.range_reads} reads)",
        )
        report.check(
            sharded_audit.shards_wanted_whole == 0,
            f"{pair.name}: no shard was wanted whole ({sharded_audit.shards_wanted_whole} were), "
            "so a shard downloaded whole is never an accepted size",
        )


def check_generated_coarse(
    pair: TwinPair, unsharded: DrivenRun, sharded: DrivenRun, cache_dir: Path | None, report: Report
) -> None:
    for run in (unsharded, sharded):
    ...
    for run in (unsharded, sharded):
        ...
        for dataset_id in run.dataset_ids:
            tier.update(generated_tier(cache_dir, dataset_id))
        tiers[run.dataset] = tier
        report.check(bool(tier), f"{pair.name}: {cache_dir} holds {len(tier)} generated chunks for {run.dataset.name}")
    first, second = tiers[unsharded.dataset], tiers[sharded.dataset]
    differing = sorted(key for key in first.keys() | second.keys() if first.get(key) != second.get(key))
    report.check(
        not differing,
        f"{pair.name}: the generated tiers are identical byte for byte"
        + ("" if not differing else f" except {len(differing)} chunks, first {differing[:3]}"),
    )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--shard", type=int, default=1024, metavar="S")
    ...
    parser.add_argument("--generated-cache-dir", type=Path, default=None, metavar="DIR")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # a dataset regenerated at a path an earlier run used is served from that
    # cache and reads nothing. Each run writes its datasets under a fresh
    # name.
    datasets = out / f"datasets-{stamp}"
    runs: dict[tuple[str, str], DrivenRun] = {}
    for pair in pairs:
        for variant, generator_args in (
            ("unsharded", pair.unsharded_args(datasets)),
            ("sharded", pair.sharded_args(datasets)),
        ):
            dataset = generate(generator_args)
            workspace_id = lucida.create_workspace(f"sharded-twins {stamp} {pair.name} {variant}")
            ...
            runs[(pair.name, variant)] = lucida.trace(
                dataset, workspace_id, out, f"{pair.name}-{variant}", args.timeout_seconds
            )
    ...
    for pair in pairs:
        ...
        if pair.generated_coarse:
            check_generated_coarse(pair, unsharded, sharded, args.generated_cache_dir, report)
