...
- feat(store): detect and parse OME-Zarr v0.5 labels at import (slipway, reversible: git-revert)
- feat(core): label overlays in the scene (off-by-default, opacity, role-aware framing) (slipway, reversible: git-revert)
- feat(web): label discoverability panel + fetch/query gating (slipway, reversible: git-revert)
- feat(web): 2D indexed-colour label rendering (value-to-colour LUT, uint32 atlas) (slipway, reversible: git-revert)
- feat(web): label picking — hover shows label id and properties (slipway, reversible: git-revert)
- feat(web): 3D volume label rendering (first-hit coloured surface) (slipway, reversible: git-revert)
- fix(web): render label ids >=65536 without 16-bit truncation (slipway, reversible: git-revert)
