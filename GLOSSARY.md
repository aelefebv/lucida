...
| **Frustum culling** | Per-chunk rejection of chunks whose AABB lies entirely outside any frustum plane, using the p-vertex method. For plate **DatasetMembers** at non-zero positions, frustum planes are offset by the member's `[pos_x, pos_y]` so the test operates in member-local coordinates | View culling, visibility test |
...
| **Upload budget** | The maximum bytes of chunk data transferred to the GPU per RAF tick for the **Minimap** (2 MB). Main view uploads are event-driven (no per-frame budget). | Frame budget, bandwidth cap |
| **Render scale** | A resolution multiplier (0.5–1.0) applied to the render target during camera interaction to maintain frame rate; restored to 1.0 after 50ms idle. Configured via `INTERACTION_RENDER_SCALE` and `FULL_RENDER_SCALE` constants | Resolution scale, quality level |
| **Offscreen target** | A single shared `rgba16float` intermediate texture. Each layer is rendered to it (with a **Scissor rect** restricting work to that well's screen bounds) and immediately composited onto the canvas before the next layer reuses it | Render target, layer buffer, FBO |
| **Scissor rect** | A screen-space bounding box computed per well by projecting the volume's `[0,1]³` AABB through `modelMatrix * viewProj`. Passed to `setScissorRect()` to restrict GPU fragment work to only pixels that could hit this well. Wells with empty scissor rects are culled entirely | Clip rect, viewport scissor |
| **Compositor** | The rendering pass that blends each **Offscreen target** onto the canvas using per-layer **BlendModes** (alpha, additive, max). Operates incrementally — one composite per layer render | Blender, combiner, merge pass |
...
| Term | Definition | Aliases to avoid |
|------|-----------|-----------------|
| **SharedChunkQueue** | A reactive cache wrapping per-member **ChunkFetchers** with `useSyncExternalStore` subscription and a configurable concurrent fetch limit (default 12, scaled by active channel count in **Multi-channel mode** via `setConcurrency()`, capped at 48) shared across all **DatasetMembers**. One instance per **Dataset** (not per member). Accepts a priority-sorted, member-qualified fetch list for cross-member spatial ordering. Uses LRU eviction with a global byte budget (`MAX_CACHE_BYTES = 512 MB`) across all members — entries are touched on `get()` (not `has()`) so chunks already in the GPU **Atlas** can be evicted; evicted chunks are transparently re-fetched by the next `ensureFetched()` call. | ChunkStore (old name), chunk cache, tile manager |
...
| **Dirty flags** | Two flags on the **Render loop** gating per-tick work: `viewDirty` (camera, contrast, visibility changes → immediate render) and `dataDirty` (chunk arrivals, intensity updates → debounced render at `DATA_RENDER_INTERVAL_MS`). The tick always runs when either flag is set (to drive chunk uploads), but the expensive render pass only executes when `shouldRender` is true. The loop quiesces when neither flag is set. | Needs redraw, invalidated |
| **Render loop** | The pull-based `requestAnimationFrame` tick that runs in two phases: (1) plan+fetch — evaluates **ChunkRequestPlans** (cached per dataset), computes seeds, builds fetch lists, submits to **SharedChunkQueue**; (2) upload+render — sends atlas config on LOD/T/C change, pushes available chunks directly to the **GPU worker**, streams fallback chunks, builds layer params, dispatches render commands. Chunks are sent directly from cache to worker (no round-trip); the main thread tracks `sentToWorker` per member, cleared on each data-render (camera stop) so the worker re-evaluates atlas allocation. Quiesces when idle. Shared planning logic lives in `tickCommon.ts`. | Frame loop, game loop, RAF loop |
