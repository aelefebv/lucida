...
| **Render loop** | The pull-based `requestAnimationFrame` tick that runs in two phases: (1) plan+fetch — evaluates **ChunkRequestPlans** (cached per dataset), computes seeds, builds fetch lists, submits to **SharedChunkQueue**; (2) upload+render — sends atlas config on LOD/T/C change, pushes available chunks directly to the **GPU worker**, streams fallback chunks, builds layer params, dispatches render commands. Chunks are sent directly from cache to worker (no round-trip); the main thread tracks `sentToWorker` per member. Quiesces when idle. Shared planning logic lives in `tickCommon.ts`. | Frame loop, game loop, RAF loop |
